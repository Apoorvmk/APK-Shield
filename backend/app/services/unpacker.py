...
def _parse_manifest_androguard(apk_path: Path) -> dict[str, Any]:
    ...
    manifest_xml_str = None
    if manifest_xml is not None:
        try:
            from lxml import etree
            manifest_xml_str = etree.tostring(manifest_xml, encoding="utf-8", pretty_print=True).decode("utf-8")
        except Exception as e:
            logger.warning(f"Failed to serialize AndroidManifest.xml: {e}")
    ...
    return {
        ...
        "declared_permissions": list(apk.get_declared_permissions() or []),
        # features & libraries
        "uses_features": list(apk.get_features() or []),
        "uses_libraries": list(apk.get_libraries() or []),
        # signing
        "signature_names": list(apk.get_signature_names() or []),
    }
